...
<body>
  ...
  <div class="main">
    ...
    $query = "SELECT DISTINCT clanci.kategorija FROM clanci WHERE clanci.arhiva = 0; ";
    $result = mysqli_query($dbc, $query);

    foreach ($result as $kat) {
      echo '<section id = "sectionArticles" class="sect">
        <div class="sectionTitle">
            <h2> <a href="kategorija.php?id=' . $kat['kategorija'] . '" class="menu"> ' . $kat['kategorija'] . ' ></a></h2>
        </div>
        <div class="story">';


      $query2 = "SELECT * FROM clanci WHERE arhiva=0 AND kategorija ='" . $kat['kategorija'] . " ' ORDER BY id DESC LIMIT 3";
